...
object GameSkin:
  ...
  def createGameSkin(name: String, base: GameSkin, load: Boolean = false): Unit =
    ...
    if load then loadGameSkin(path)
  ...
  def loadGameSkin(path: String, makeClone: Boolean = true): Unit =
